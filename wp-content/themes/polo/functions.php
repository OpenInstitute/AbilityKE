...
function addScripts() {
	...
	wp_enqueue_style( 'slider', get_template_directory_uri() . '/assets/timeline/css/timeline.css', array(), '1.1', 'all');
   
	wp_enqueue_script( 'script', get_template_directory_uri() . '/assets/timeline/js/timeline.js', array ( 'jquery' ), 1.1, true);
...
// Add a function to register the scripts from CDN

function responsiveScripts(){
	wp_enqueue_style('oi-css-responsive', 'https://cdn.openinstitute.io/css/oi-responsive-min.css', NULL, false, 'all' );
	wp_register_script('oi-jquery', 'https://cdn.openinstitute.io/js/oi-jquery.min.js', array('jquery'), false, true);
	wp_enqueue_script('oi-jquery');
}

add_action('wp_enqueue_scripts', 'responsiveScripts');

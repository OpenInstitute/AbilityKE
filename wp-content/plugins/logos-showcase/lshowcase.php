...
// Last modified: Feb 23rd 2018

// Next Edits:
// child menu entries on isotope menu filter not working properly
// translation files
// Pagination / Load More
// child-parent hierachy on shortcode generator drodown
// improve or disable slider reset after window resize
// hover layout
...
function shortcode_lshowcase($atts)
{
	...
	$html = build_lshowcase($orderby, $category, $tag, $activeurl, $style, $interface, $tooltip, $description, $limit, $slidersettings,$img,$ids,$filter,$class,$relation,$exclude);
...
function build_lshowcase($order = "menu_order", $category = "", $tag = "", $activeurl = "new", $style = "normal", $interface = "grid", $tooltip = "false", $description = "false", $limit = - 1, $slidersettings="", $imgwo=0, $ids="0", $filter="false", $custom_class='', $relation = 'OR', $exclude = false)
{
	...
	while ($loop->have_posts()):
		...
		if (has_post_thumbnail() || $lshowcase_options['lshowcase_default_image'] != '' || $custom_img_url != ''):
			...
			if ($interface != "hcarousel" ) {
				$html.= "<div data-logo-id='".get_the_ID()."' class='" . $divwrap . $cat . "'>";
			...
			if ($interface == "grid" ) {
				...
				$html.= '<div data-logo-id="'.get_the_ID().'" class="' . $divboxinnerclass . '">';
			}

...
ORDER BY CONVERT(`Value`, DECIMAL(25,2))  DESC limit 0, 25;";	
...
/* ------------------------------------------------------------------------------ */		
	...
	function get_countyBudgetPeriod($id, $period = '', $latest='')
	{
		...
		if($id) 
		{
			$cCounty_clean		= clean_title($this->get_countyProfile($id)['county']);
			
			$sq_qry = "SELECT TRIM(period) AS `period` , TRIM(county) AS `county` FROM `oc_cob_budgets` WHERE (TRIM(county) LIKE ".$this->quote_si($cCounty_clean,1).") GROUP BY `period` ORDER BY `period` DESC ".$limit.";";
			//echo $sq_qry; exit;
			if($latest == 1) {
...
ORDER BY `Sector` ASC, `Period` DESC; ";	
	...
	function get_countySectorCardsNew($county, $selperiod = '', $pdf = '')
	{
		...
		$sq_pdf = "";
		if($pdf == 'y') { 
			$sq_pdf = " OR `county` LIKE ".$this->quote_si($county, 1)."  and `period`  LIKE '%2012%' or `county` LIKE ".$this->quote_si($county, 1)."  and `period`  LIKE '%2011%' or `county` LIKE ".$this->quote_si($county, 1)."  and `period`  LIKE '%2010%'  or `county` LIKE ".$this->quote_si($county, 1)."  and `period`  LIKE '%2009%' "; 
		}
...
/* ------------------------------------------------------------------------------ */	
	...
	function get_Constituency($psource, $pconst = 0)
	{
		$result 	= array();		
		...
		if($this->recordCount($rs_qry) > 0)
		{
			while($cn_qry_a = $this->fetchRow($rs_qry, 'assoc'))
			{
				...
				if(intval($pconst) == $region_code) { $active=' selected '; } 

...
class drop_downs extends master
{
	...
	// This function gets all fields from the buildings
	function zoner(){
		// $result=$this->dbQuery("SELECT DISTINCT zone_no, area FROM buildings where zone_no =35");
		$result=$this->dbQuery("SELECT DISTINCT zone_no, area FROM buildings");
			$records = $this->recordCount($result);
			if($records >=1 )	
			{
				$i=0;
				while($qry_data = $this->fetchRow($result))
				{	
					//echo $qry_data['entity_code'];

					// <option value="1">One</option>
					$metadata = '<strong>'.$qry_data['zone_no'].' &middot; '.$qry_data['area'].'</strong>';
					echo '<option value="'.$qry_data['zone_no'].'">'.$metadata.'</option>';
				$i += 1;
				}
	...
	function abilityDrop($type, $zone){ 
		if($zone == ''){
			$zone = '';
		}		
		//$this->connect() or trigger_error('SQL', E_USER_ERROR);
		if($type == 'indoor'){
			$query = "SELECT * FROM buildings where zone_no = $zone AND published=1 order by entity_code";
			$result=$this->dbQuery($query);
			$records = $this->recordCount($result);
			if($records >=1 )	
			{
				$out = '';
				$out .=  '<option value=""> Select from zone '.$zone.' </option>';
				while($qry_data = $this->fetchRow($result))
				{	
					$metadata = '<strong>'.$qry_data['entity_code'].'</strong> &middot; <em>'.$qry_data['building_name'].' &middot; '.$qry_data['sub_building'].'</em>';
					$out .=  '<option value="'.$qry_data['entity_code'].'">'.$metadata.'</option>';
		...
		// Special case for the #UniversityMappingChallenge
		if($type == 'umc-indoor'){
			$query = "SELECT * FROM buildings where zone_no = $zone AND published = 1  order by entity_code";
			$result=$this->dbQuery($query);
			$records = $this->recordCount($result);
		...
		if($type == 'outdoor'){
			$sq_more = "";
			$qry = "SELECT * FROM `streets` where zone = $zone";
			...
			if($records >=1 )	
			{
				...
				while($qry_data = $this->fetchRow($result))
				{	
					$metadata = '<strong>'.$qry_data['street_code'].'</strong> &middot; <em>'.$qry_data['road'].'</em>';
					$out .= '<option value="'.$qry_data['street_code'].'">'.$metadata.'</option>';

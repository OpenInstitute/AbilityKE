		...
		if(is_array($menu))
		{
			foreach($menu as $key => $val )	//=> $ml
			{
				...
				if( is_array($ml)) 
				{
						...
						elseif( $link <> "#") 
						{ 	
							if($link == 'factsheet.php' or $link == 'factsheet_all.php') { $target = ' target="_blank" '; }
							$link = $link . '?com='.$key; 
						  	//$link = $link_seo;
						}
					...
					if($ml['to_footer'] == 1 or $ml['id_menu_type'] == 5) { 
						if (!array_key_exists($key, $this->linkToFoot)) {
							$this->linkToFoot[$key] = "<li><a $linkb  class=\"". $isActive ."\" ".$target.">".$ml['title']."</a></li>"; 
						}
					}
...
ORDER BY `oc_dt_downloads`.`date_updated` DESC;";
	...
	***********************************************************/
	...
	function checklistCategory($cat){
		// define output
		$out = '';
		// Get a list of questions from the category
		$sql = 'SELECT * FROM `ability_checklist_questions` where category = "'.$cat.'" ';
		...
		// Run a while loop to iterate through the questions
		while($data = $this->FetchRow($res)){

			$out .= '  <div class="form-group">
			<label class="col-md-4" for="building_type">'.$data['questions'].'</label>
				<div class="col-md-8">
					<input type="radio" name="'.$data['name'].'" class="'.$data['name'].'" value="yes" class=""> Yes
					<input type="radio" name="'.$data['name'].'" class="'.$data['name'].'" value="public" class=""> No
					<input type="radio" name="'.$data['name'].'" class="'.$data['name'].'" value="public" class=""> Not Applicable
				</div> 
			</div>';
		}

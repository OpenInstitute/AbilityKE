...
function niceNumber($num, $adjust = '1', $codeStyle = 'txt75', $decs = 2) { 
	$n = floatval($num);
	
	$trillion = ( $adjust == '1') ? ' trillion' : 't';
	$billion = ( $adjust == '1') ? ' billion' : 'b';
	$million = ( $adjust == '1') ? ' million' : 'm';
	$thousand = ( $adjust == '1') ? ' thousand' : 'k';
	
	$styl = ' class="'. $codeStyle .'" ';
	
	if($n>1000000000000) return round(($n/1000000000000),$decs).'<span '.$styl.'>'.$trillion.'</span>';
	else if($n>1000000000) return round(($n/1000000000),$decs).'<span'.$styl.'>'.$billion.'</span>';
	else if($n>1000000) return round(($n/1000000),$decs).'<span'.$styl.'>'.$million.'</span>';
	else if($n>1000) return round(($n/1000),$decs).'<span'.$styl.'>'.$thousand.'</span>';
    
	return number_format($n,$decs);          
}
...
function displayDecimal($amount, $decs=2) { $out = ($amount <> '') ? number_format(floatval($amount),$decs,'.',',') : '' ; return $out; }
...
function clean_output($str, $useBreak=0)
{
	...
	$str = mb_convert_encoding($str, 'UTF-8', 'UTF-8'); 
	$str = iconv("ISO-8859-15", "UTF-8//TRANSLIT", iconv("UTF-8", "ISO-8859-15//TRANSLIT", $str));
...
function clean_alphaonly($string, $nospace=false)
{
	$string 		= trim(html_entity_decode(stripslashes($string))); //strtolower()
